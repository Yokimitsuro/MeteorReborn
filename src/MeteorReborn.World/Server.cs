// PM-COMPLETE → verbatim port de World Server/Server.cs (541 líneas).
...
using MeteorReborn.World.Packets.WorldPackets.Receive;
using MeteorReborn.World.Packets.WorldPackets.Receive.Group;
...
namespace MeteorReborn.World
{
    class Server
    {
        ...
        public bool StartServer()
        {
            ...
            Log.Information("World Server accepting connections @ {0}:{1}", (mServerSocket.LocalEndPoint as IPEndPoint).Address, (mServerSocket.LocalEndPoint as IPEndPoint).Port);
                ...
                switch (subpacket.gameMessage.opcode)
                {
                    ...
                    case 0x1000:
                        ...
                        if (beginConfirmPacket.invalidPacket || beginConfirmPacket.errorCode == 0)
                            Log.Error("Session {0} had a error beginning session.", beginConfirmPacket.sessionId);
                    ...
                    case 0x1001:
                        ...
                        if (!endConfirmPacket.invalidPacket && endConfirmPacket.errorCode == 0)
                        {
                            ...
                            Log.Error("Session {0} had an error ending session.", endConfirmPacket.sessionId);
