// PM-COMPLETE → verbatim port de World Server/DataObjects/ZoneServer.cs (188 líneas).
...
using Serilog;
using MeteorReborn.World.Packets.WorldPackets.Send;

namespace MeteorReborn.World.DataObjects
{
    class ZoneServer
    {
        ...
        public bool Connect()
        {
            Log.Information("Connecting to zone server @ {0}:{1}", zoneServerIp, zoneServerPort);
            // LANG-ADAPT: PM usaba IPAddress.Parse (solo IPs literales). Para soportar hostnames
            // (e.g. `host.docker.internal` en Docker) hacemos resolución DNS si no parsea como IP.
            IPAddress ip = IPAddress.TryParse(zoneServerIp, out var parsed)
                ? parsed
                : Dns.GetHostAddresses(zoneServerIp).First(a => a.AddressFamily == AddressFamily.InterNetwork);
            IPEndPoint remoteEP = new IPEndPoint(ip, zoneServerPort);
            ...
            { Log.Error("Failed to connect"); return false; }
        ...
        public void SendPacket(SubPacket subpacket)
        {
            if (isConnected)
            {
                ...
                { Log.Error(e, "Weird case, socket was d/ced: {0}"); }
            }
            else
            {
                if (Connect())
        ...
        private void ReceiveCallback(IAsyncResult result)
        {
            ...
            if ((conn.socket.Poll(1, SelectMode.SelectRead) && conn.socket.Available == 0))
            {
                conn = null;
                isConnected = false;
                Log.Information("Zone server @ {0}:{1} disconnected!", zoneServerIp, zoneServerPort);
                return;
            }
