// PM-COMPLETE → verbatim port de World Server/WorldMaster.cs (535 líneas).
...
using MeteorReborn.World.Packets.Send.Subpackets;
using MeteorReborn.World.Packets.Send.Subpackets.Groups;
using MeteorReborn.World.Packets.WorldPackets.Send;
using MeteorReborn.World.Packets.WorldPackets.Send.Group;
using Npgsql;
using NpgsqlTypes;
...
namespace MeteorReborn.World
{
    class WorldManager
    {
                ...
                try
                {
                    ...
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            ZoneEntrance entance = new ZoneEntrance(reader.GetUInt32(1), privArea, reader.GetByteAt(2), reader.GetFloat(3), reader.GetFloat(4), reader.GetFloat(5), reader.GetFloat(6));
        ...
        {
            Log.Information("--------------------------");
            Log.Information("Connecting to zone servers");
            Log.Information("--------------------------");
