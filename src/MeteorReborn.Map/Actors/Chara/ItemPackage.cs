// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/ItemPackage.cs (930 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.player
{
    class ItemPackage
    {       
        ...
        public int AddItem(uint itemId, int quantity, byte quality)
        {
            ...
            if (gItem == null)
            {
                Log.Error("Inventory.AddItem: unable to find item %u", itemId);
                return ERROR_SYSTEM;
            }
        ...
        public int AddItems(uint[] itemIds, uint[] quantity = null, byte[] quality = null)
        {
            ...
            for (int i = 0; i < itemIds.Length; i++)
            {
                ...
                if (gItem == null)
                {
                    Log.Error("Inventory.AddItem: unable to find item %u", itemIds[i]);
                    return ERROR_SYSTEM;
                }
        ...
        public int AddItem(InventoryItem itemRef)
        {
            ...
            if (gItem == null)
            {
                Log.Error("Inventory.AddItem: unable to find item %u", itemRef.itemId);
                return ERROR_SYSTEM;
            }
