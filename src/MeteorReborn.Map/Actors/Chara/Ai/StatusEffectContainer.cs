// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/StatusEffectContainer.cs (445 líneas).
            ...
            if (canOverwrite || effect == null)
            {
                ...
                if (effects.Count < MAX_EFFECTS)
                {
                    ...
                    if (!newEffect.GetHidden())
                    {
                        ...
                        uint time = (newEffect.GetFlags() & (uint) StatusEffectFlags.Stance) == 0 ? MeteorReborn.Common.Utils.UnixTimeStampUTC(newEffect.GetEndTime()) : 0xFFFFFFFF;
