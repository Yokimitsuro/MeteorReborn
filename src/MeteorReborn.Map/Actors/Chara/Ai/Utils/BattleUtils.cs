// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/Utils/BattleUtils.cs (924 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.Ai.Utils
{
    static class BattleUtils
    {
        ...
        {
            if ((Random.Shared.NextDouble() * 100) <= action.critRate)
            {
        ...
        public static bool TryResist(Character attacker, Character defender, BattleCommand skill, CommandResult action)
        {
            ...
            while ((Random.Shared.NextDouble() * 100) <= rate && i < 4)
            {
                rate /= 2;
                i++;
            }
        ...
        {
            if ((Random.Shared.NextDouble() * 100) <= action.blockRate)
            {
        ...
        {
            if ((Random.Shared.NextDouble() * 100) <= action.parryRate)
            {
        ...
        {
            if ((Random.Shared.NextDouble() * 100) >= GetHitRate(attacker, defender, skill, action))
            {
