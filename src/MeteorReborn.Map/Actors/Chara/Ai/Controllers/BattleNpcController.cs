// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/Controllers/BattleNpcController.cs (430 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.Ai.Controllers
{
    class BattleNpcController : Controller
    {
        ...
        public override void Disengage()
        {
            ...
            Lua.LuaEngine.CallLuaBattleFunction(owner, "onDisengage", owner, target, MeteorReborn.Common.Utils.UnixTimeStampUTC(lastUpdate));
        }
        ...
        protected virtual void DoCombatTick(DateTime tick, List<Character> contentGroupCharas = null)
        {
            ...
            if ((tick - lastCombatTickScript).TotalSeconds > 3)
            {
                ...
                //Lua.LuaEngine.CallLuaBattleFunction(owner, "onCombatTick", owner, owner.target, MeteorReborn.Common.Utils.UnixTimeStampUTC(tick), contentGroupCharas);
        ...
        protected virtual void Move()
        {
            ...
            vecToTarget = (MeteorReborn.Common.Utils.Distance(owner.GetPosAsVector3(), owner.target.GetPosAsVector3()) - owner.GetAttackRange() + 0.2f) * vecToTarget;

            var targetPos = vecToTarget + owner.GetPosAsVector3();
            var distance = MeteorReborn.Common.Utils.Distance(owner.GetPosAsVector3(), owner.target.GetPosAsVector3());
                    ...
                    if (!owner.aiContainer.pathFind.IsFollowingPath())
                    {
                        if (owner.target is Player)
                        {
                            foreach (var chara in owner.zone.GetActorsAroundActor<Character>(owner, 1))
                            {
                                ...
                                float mobDistance = MeteorReborn.Common.Utils.Distance(owner.positionX, owner.positionY, owner.positionZ, chara.positionX, chara.positionY, chara.positionZ);
        ...
        public virtual bool CanDetectTarget(Character target, bool forceSight = false)
        {
            ...
            var distance = MeteorReborn.Common.Utils.Distance(owner.positionX, owner.positionY, owner.positionZ, target.positionX, target.positionY, target.positionZ);
