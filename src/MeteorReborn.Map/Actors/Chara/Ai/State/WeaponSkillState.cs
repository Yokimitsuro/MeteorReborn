// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/State/WeaponSkillState.cs (199 líneas).
                ...
                if ((skill.positionBonus & Utils.BattleUtils.ConvertHitDirToPosition(hitDirection)) == skill.positionBonus)
                {
                    ...
                    if (!skill.IsInstantCast())
                    {
                        ...
                        if (owner is Player)
                        {
                            ...
                            ((Player)owner).SendStartCastbar(skill.id, MeteorReborn.Common.Utils.UnixTimeStampUTC(DateTime.Now.AddMilliseconds(castTime)));
                        }
