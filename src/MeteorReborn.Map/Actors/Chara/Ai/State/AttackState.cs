// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/State/AttackState.cs (232 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.Ai.state
{
    class AttackState : State
    {
        ...
        private bool CanAttack()
        {
            ...
            else if (MeteorReborn.Common.Utils.Distance(owner.positionX, owner.positionY, owner.positionZ, target.positionX, target.positionY, target.positionZ) > owner.GetAttackRange())
            {
                if (owner is Player)
                {
