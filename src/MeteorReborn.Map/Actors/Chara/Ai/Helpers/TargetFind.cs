// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/Helpers/TargetFind.cs (509 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.Ai
{
    ...
    class TargetFind
    {
        ...
        {
            double distance = MeteorReborn.Common.Utils.XZDistance(aoeTargetPosition, target.GetPosAsVector3());
        ...
        private void AddAllInHateList()
        {
            ...
            {
                Log.Error($"TargetFind.AddAllInHateList() owner [{owner.actorId}] {owner.customDisplayName} {owner.actorName} is not a BattleNpc");
            }
            else
            {
