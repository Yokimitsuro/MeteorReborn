// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Ai/Helpers/PathFind.cs (227 líneas).
...
namespace MeteorReborn.Map.Actors.Chara.Ai
{
    ...
    class PathFind
    {
        ...
        public void PreparePath(float x, float y, float z, float stepSize = 1.25f, int maxPath = 40, float polyRadius = 0.0f)
        {
            ...
            if (path != null)
            {
                ...
                //if (path.Count == 1)
                //    Log.Information($"mypos: {owner.positionX} {owner.positionY} {owner.positionZ} | targetPos: {x} {y} {z} | step {stepSize} | maxPath {maxPath} | polyRadius {polyRadius}");

                //Log.Error("[{0}][{1}] Created {2} points in {3} milliseconds", owner.actorId, owner.actorName, path.Count, sw.ElapsedMilliseconds);
            }
        }
        ...
        public void FollowPath()
        {
            if (path?.Count > 0)
            {
                ...
                if (AtPoint(point))
                {
                    path.Remove(point);
                    owner.OnPath(point);
                    //Log.Error($"{owner.actorName} arrived at point {point.X} {point.Y} {point.Z}");
                }
        ...
        public bool AtPoint(Vector3 point = null)
        {
            ...
            if (distanceFromPoint == 0)
                ...
                return MeteorReborn.Common.Utils.Distance(owner.positionX, owner.positionY, owner.positionZ, point.X, point.Y, point.Z) <= (distanceFromPoint + 4.5f);
        }
        ...
        public void StepTo(Vector3 point, bool run = false)
        {
            ...
            float distanceTo = MeteorReborn.Common.Utils.Distance(owner.positionX, owner.positionY, owner.positionZ, point.X, point.Y, point.Z);
