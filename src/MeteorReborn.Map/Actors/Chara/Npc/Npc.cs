// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Npc/Npc.cs (478 líneas).
...
namespace MeteorReborn.Map.Actors
{
    ...
    class Npc : Character
    {
        ...
        public void LoadNpcAppearance(uint id)
        {
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Server={0}; Port={1}; Database={2}; UID={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    cmd.Parameters.AddParam("@templateId", (long)id);
        ...
        {
            // LANG-ADAPT: PM almacenaba JSON con escapes MySQL (\r\n, \", \\). MySqlConnector
            // los procesaba al leer. Postgres con standard_conforming_strings=on preserva los
            // backslashes literales → Newtonsoft falla. Regex.Unescape revierte los escapes.
            var unescaped = System.Text.RegularExpressions.Regex.Unescape(eventConditions);
            EventList conditions = JsonConvert.DeserializeObject<EventList>(unescaped);
