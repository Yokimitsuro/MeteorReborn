// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Npc/BattleNpc.cs (465 líneas).
...
namespace MeteorReborn.Map.Actors
{
    ...
    class BattleNpc : Npc
    {
        ...
        public override bool CanUse(Character target, BattleCommand spell, CommandResult error = null)
        {
            ...
            if (MeteorReborn.Common.Utils.Distance(positionX, positionY, positionZ, target.positionX, target.positionY, target.positionZ) > spell.range)
            {
        ...
        {
            return this.isAtSpawn = MeteorReborn.Common.Utils.DistanceSquared(positionX, positionY, positionZ, spawnX, spawnY, spawnZ) <= 2500.0f;
        }
