// PM-COMPLETE → verbatim port de Map Server/Actors/Chara/Player/Player.cs (2933 líneas).
...
using MeteorReborn.Map.DataObjects.Chara;
...
using MeteorReborn.Map.Packets.Receive.Events;
...
namespace MeteorReborn.Map.Actors
{
    class Player : Character
    {
        ...
        public Player(Session cp, uint actorID) : base(actorID)
        {
            ...
            lastPlayTimeUpdate = MeteorReborn.Common.Utils.UnixTimeStampUTC();
        ...
        */
        ...
        private byte GetInnCode()
        {
            if (zone.isInn)
            {
                ...
                if (MeteorReborn.Common.Utils.Distance(position, new Vector3(0, 0, 0)) <= 20f)
                    return 3;
                else if (MeteorReborn.Common.Utils.Distance(position, new Vector3(160, 0, 160)) <= 20f)
                    return 2;
                else if (MeteorReborn.Common.Utils.Distance(position, new Vector3(-160, 0, -160)) <= 20f)
                    return 1;
            }
        ...
        public uint GetPlayTime(bool doUpdate)
        {
            if (doUpdate)
            {
                uint curTime = MeteorReborn.Common.Utils.UnixTimeStampUTC();
        ...
        public void SendCharaExpInfo()
        {
            if (lastStep == 0)
            {
                ...
                if (maxLength == 0x5E)
                {
                    charaInfo1.AddBuffer(MeteorReborn.Common.Utils.MurmurHash2("charaWork.battleSave.skillLevel", 0), skillLevelBuffer, 0, skillLevelBuffer.Length, 0x0);
                    lastPosition += maxLength;
                }
                else
                {
                    charaInfo1.AddBuffer(MeteorReborn.Common.Utils.MurmurHash2("charaWork.battleSave.skillLevel", 0), skillLevelBuffer, 0, skillLevelBuffer.Length, 0x3);
                    lastPosition = 0;
                    lastStep++;
                }
            ...
            else if (lastStep == 1)
            {
                ...
                {
                    charaInfo1.SetIsArrayMode(true);
                    charaInfo1.AddBuffer(MeteorReborn.Common.Utils.MurmurHash2("charaWork.battleSave.skillLevelCap", 0), skillCapBuffer, 0, skillCapBuffer.Length, 0x1);
                    lastPosition += maxLength;
                }
                else
                {
                    charaInfo1.SetIsArrayMode(false);
                    charaInfo1.AddBuffer(MeteorReborn.Common.Utils.MurmurHash2("charaWork.battleSave.skillLevelCap", 0), skillCapBuffer, 0, skillCapBuffer.Length, 0x3);
                    lastStep = 0;
                    lastPosition = 0;
                }
        ...
        public void DoClassChange(byte classId)
        {
            ...
            var time = MeteorReborn.Common.Utils.UnixTimeStampUTC();
            for(int i = charaWork.commandBorder; i < charaWork.command.Length; i++)
            {
                if(charaWork.command[i] != 0)
                {
        ...
        {
            rentalExpireTime = MeteorReborn.Common.Utils.UnixTimeStampUTC() + ((uint)numMins * 60);
        ...
        public override void Update(DateTime tick)
        {
            ...
            if (rentalExpireTime != 0)
            {
                uint tickUTC = MeteorReborn.Common.Utils.UnixTimeStampUTC(tick);
        ...
        public void EquipAbility(byte classId, uint commandId, ushort hotbarSlot, bool printMessage = true)
        {
            ...
            uint recastEnd = MeteorReborn.Common.Utils.UnixTimeStampUTC() + maxRecastTime;
        ...
        private void UpdateHotbarTimer(uint commandId, uint recastTimeMs)
        {
            ...
            charaWork.parameterSave.commandSlot_recastTime[slot - charaWork.commandBorder] = MeteorReborn.Common.Utils.UnixTimeStampUTC(DateTime.Now.AddMilliseconds(recastTimeMs));
        ...
        public override bool CanUse(Character target, BattleCommand skill, CommandResult error = null)
        {
            ...
            if (GetHotbarTimer(skill.id) > MeteorReborn.Common.Utils.UnixTimeStampUTC())
            {
            ...
            if (MeteorReborn.Common.Utils.XZDistance(positionX, positionZ, target.positionX, target.positionZ) > skill.range)
            {
            ...
            if (MeteorReborn.Common.Utils.XZDistance(positionX, positionZ, target.positionX, target.positionZ) < skill.minRange)
            {
