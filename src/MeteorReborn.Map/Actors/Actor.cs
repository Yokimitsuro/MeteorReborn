...
namespace MeteorReborn.Map.Actors
{
    ...
    class Actor
    {
        ...
        public void GenerateActorName(int actorNumber)
        {
            ...
            string classNumber = MeteorReborn.Common.Utils.ToStringBase63(actorNumber);
        ...
        public void LookAt(Actor actor)
        {
            ...
            {
                Log.Error("[{0}][{1}] Actor.LookAt() unable to find actor!", actorId, actorName);
            }
        }
        ...
        public bool IsFacing(Actor target, float angle = 40.0f)
        {
            if (target == null)
            {
                Log.Error("[{0}][{1}] IsFacing no target!", actorId, actorName);
                return false;
            }
        ...
        public Vector3 FindRandomPoint(float x, float y, float z, float minRadius, float maxRadius)
        {
            ...
            var radius = Math.Sqrt(Random.Shared.NextDouble() * (maxRadius - minRadius)) + minRadius;
        ...
        public Vector3 FindRandomPointAroundTarget(Actor target, float minRadius, float maxRadius)
        {
            if (target == null)
            {
                Log.Error(String.Format("[{0} {1}] FindRandomPointAroundTarget: no target found!", this.actorId, this.customDisplayName));
