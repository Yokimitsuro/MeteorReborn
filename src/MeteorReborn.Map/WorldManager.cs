// PM-COMPLETE → verbatim port de Map Server/WorldManager.cs (2013 líneas).
...
using System.Linq;
using MeteorReborn.Map.Packets.WorldPackets.Receive;
using MeteorReborn.Map.Packets.WorldPackets.Send.Group;
using System.Threading;
using MeteorReborn.Map.Actors.Chara.Ai;
using MeteorReborn.Map.Actors.Chara;
using MeteorReborn.Map.Packets.Send.Actor.Inventory;
...
namespace MeteorReborn.Map
{
    class WorldManager
    {
        ...
        public void LoadZoneList()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Server={0}; Port={1}; Database={2}; UID={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    cmd.Parameters.AddParam("@ip", ConfigConstants.OPTIONS_BINDIP);
                    // LANG-ADAPT: PM relied on MySQL implicit string→int. Postgres bigint = text fails.
                    cmd.Parameters.AddParam("@port", (long)int.Parse(ConfigConstants.OPTIONS_PORT));
            ...
            Log.Information(String.Format("Loaded {0} zones and {1} private areas.", count1, count2));
        }
        ...
        public void LoadZoneEntranceList()
        {
                ...
                try
                {
                    ...
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            ZoneEntrance entance = new ZoneEntrance(reader.GetUInt32(1), privArea, 1, reader.GetByteAt(2), reader.GetFloat(3), reader.GetFloat(4), reader.GetFloat(5), reader.GetFloat(6));
            ...
            Log.Information(String.Format("Loaded {0} zone spawn locations.", count));
        }
        ...
        public void LoadSeamlessBoundryList()
        {
            ...
            Log.Information(String.Format("Loaded {0} region seamless boundries.", count));
        }
        ...
        {            
            ...
            Log.Information(String.Format("Loaded {0} actor classes.", count));
        }
        ...
        public void LoadSpawnLocations()
        {
            ...
            Log.Information(String.Format("Loaded {0} spawn(s).", count));
        }
        ...
        public void LoadBattleNpcs()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Server={0}; Port={1}; Database={2}; UID={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                ...
                {
                    conn.Open();
                    // LANG-ADAPT: PM usa `GROUP BY bsl.bnpcId` MySQL-style (permisivo, devuelve
                    // fila arbitraria por grupo). Postgres es estricto y exige todas las cols.
                    // `DISTINCT ON (col)` Postgres reproduce el comportamiento MySQL exacto.
                    var query = @"
                    SELECT DISTINCT ON (bsl.bnpcId) bsl.bnpcId, bsl.groupId, bsl.positionX, bsl.positionY, bsl.positionZ, bsl.rotation,
                    ...
                    foreach (var zonePair in zoneList)
                    {
                        ...
                        NpgsqlCommand cmd = new NpgsqlCommand(query, conn);
                        // LANG-ADAPT: Npgsql 9 no acepta uint sin NpgsqlDbType explícito → cast a long (bigint).
                        cmd.Parameters.AddParam("@zoneId", (long)zonePair.Key);
                        ...
                        using (NpgsqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                battleNpc.charaWork.parameterSave.state_mainSkillLevel = (short)Random.Shared.Next(reader.GetByte("minLevel"), reader.GetByte("maxLevel"));
                    ...
                    Log.Information("Loaded {0} monsters.", count);
                }
                catch (NpgsqlException e)
                {
        ...
        {
            Log.Information("Spawning actors...");
        ...
        public BattleNpc SpawnBattleNpcById(uint id, Area area = null)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Server={0}; Port={1}; Database={2}; UID={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                ...
                {
                    conn.Open();
                    // LANG-ADAPT: PM GROUP BY MySQL-style → Postgres `DISTINCT ON` (mismo comportamiento).
                    var query = @"
                    SELECT DISTINCT ON (bsl.bnpcId) bsl.bnpcId, bsl.groupId, bsl.positionX, bsl.positionY, bsl.positionZ, bsl.rotation,
                    bgr.groupId, bgr.poolId, bgr.scriptName, bgr.minLevel, bgr.maxLevel, bgr.respawnTime, bgr.hp, bgr.mp,
                    bgr.dropListId, bgr.allegiance, bgr.spawnType, bgr.animationId, bgr.actorState, bgr.privateAreaName, bgr.privateAreaLevel, bgr.zoneId,
                    ...
                    INNER JOIN server_battlenpc_pools bpo ON bgr.poolId = bpo.poolId
                    INNER JOIN server_battlenpc_genus bge ON bpo.genusId = bge.genusId
                    WHERE bsl.bnpcId = @bnpcId;
                    ";

                    var count = 0;

                    NpgsqlCommand cmd = new NpgsqlCommand(query, conn);
                    cmd.Parameters.AddParam("@bnpcId", id);
                    ...
                    Log.Information("WorldManager.SpawnBattleNpcById spawned BattleNpc {0}.", id);
                }
                catch (NpgsqlException e)
                {
                    Log.Error(e.ToString());
        ...
        {       
            ...
            if (newArea == null)
            {
                Log.Debug("Request to change to zone not on this server by: {0}.", player.customDisplayName);
        ...
        public void DoZoneChangeContent(Player player, PrivateAreaContent contentArea, float spawnX, float spawnY, float spawnZ, float spawnRotation, ushort spawnType = SetActorPositionPacket.SPAWNTYPE_WARP_DUTY)
        {
            ...
            if (contentArea == null)
            {
                Log.Debug("Request to change to content area not on this server by: {0}.", player.customDisplayName);
                return;
            }
        ...
        public void StartZoneThread()
        {
            ...
            Log.Information("Zone Loop has started");
        }
