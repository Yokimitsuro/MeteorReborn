// PM-COMPLETE → verbatim port de Map Server/Lua/LuaEngine.cs (870 líneas).
...
namespace MeteorReborn.Map.Lua
{
    class LuaEngine
    {
        ...
        {
            ulong time = MeteorReborn.Common.Utils.MilisUnixTimeStampUTC() + (ulong)(seconds * 1000);
        ...
        {
            ulong currentTime = MeteorReborn.Common.Utils.MilisUnixTimeStampUTC();
        ...
        public static void CallLuaBattleFunction(Character actor, string functionName, params object[] args)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Log.Error($"LuaEngine.CallLuaBattleFunction [{functionName}] {e.Message}");
                }
        ...
        public static int CallLuaStatusEffectFunction(Character actor, StatusEffect effect, string functionName, params object[] args)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Log.Error($"LuaEngine.CallLuaStatusEffectFunction [{functionName}] {e.Message}");
                }
                DynValue res = new DynValue();
            ...
            {
                Log.Error($"LuaEngine.CallLuaStatusEffectFunction [{effect.GetName()}] Unable to find script {path}");
            }
            return -1;
        }
        ...
        public static int CallLuaBattleCommandFunction(Character actor, BattleCommand command, string folder, string functionName, params object[] args)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Log.Error($"LuaEngine.CallLuaBattleCommandFunction [{functionName}] {e.Message}");
                }
                DynValue res = new DynValue();
            ...
            else
            {
                ...
                //Log.Error($"LuaEngine.CallLuaBattleCommandFunction [{command.name}] Unable to find script {path}");
                var script = LoadGlobals();
        ...
        public static void LoadBattleCommandScript(BattleCommand command, string folder)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Log.Error($"LuaEngine.CallLuaBattleCommandFunction {e.Message}");
                }
        ...
        public static void RunGMCommand(Player player, String cmd, string[] param, bool help = false)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Log.Error("LuaEngine.RunGMCommand: {0}.", e.Message);
                    return;
                }
                ...
                if (!script.Globals.Get("onTrigger").IsNil())
                {
                    ...
                    if (player != null)
                    {
                        ...
                        {
                            Log.Information("LuaEngine.RunGMCommand: {0}'s GM level is too low to use command {1}.", player.actorName, cmd);
                            return;
                        }
                    ...
                    {
                        LuaScript.Log.Information("[Commands] [{0}]: {1}", cmd, description);
                        return;
                    }
                        ...
                        try
                        {
                            ...
                            switch (parameters[i])
                            {
                                ...
                                case 's':
                                    ...
                                    continue;
                                default:
                                    LuaScript.Log.Information("LuaEngine.RunGMCommand: {0} unknown parameter {1}.", path, parameters[i]);
                                    LuaParam.Add(param[i + 1]);
                                    continue;
                            }
                        }
                    ...
                    {
                        Log.Error("LuaEngine.RunGMCommand: {0} - {1}", path, e.Message);
                    }
                    return;
                }
            }
        ...
        public static LuaScript LoadScript(string path)
        {
            ...
            {
                Log.Error("{0}.", e.DecoratedMessage);
                return null;
            }
            return script;
        }
        ...
        public static LuaScript LoadGlobals(LuaScript script = null)
        {
            ...
            // LANG-ADAPT: en Linux el lookup de require() es case-sensitive. Reemplazar el
            // default loader por uno que baja modname a lowercase (PM-faithful con Windows).
            var loader = new CaseInsensitiveScriptLoader();
            loader.ModulePaths = FileSystemScriptLoader.UnpackStringPaths("./scripts/?;./scripts/?.lua");
