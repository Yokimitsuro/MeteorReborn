// PM-COMPLETE → verbatim port de Map Server/ConfigConstants.cs (114 líneas).
...
namespace MeteorReborn.Map
{
    class ConfigConstants
    {
        ...
        {
            Log.Information("Loading map_config.ini file... ");
        ...
        public static void ApplyLaunchArgs(string[] launchArgs)
        {
            ...
            for (var i = 0; i + 1 < args.Count; i += 2)
            {
                ...
                if (!legit)
                {
                    Log.Error("Invalid parameter <{0}> for argument: <--{1}> or argument doesnt exist!", val, arg);
                }
            }
        }
    }
