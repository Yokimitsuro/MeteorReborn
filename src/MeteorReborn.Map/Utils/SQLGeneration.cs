        ...
        public static void GenerateZones()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Host={0}; Port={1}; Database={2}; Username={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    cmd.Parameters.AddParam("@id", 100);
                    cmd.Parameters.AddParam("@placename", "");
        ...
        public static void GenerateActors()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Host={0}; Port={1}; Database={2}; Username={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    NpgsqlCommand cmd = new NpgsqlCommand(query, conn);
                    cmd.Parameters.AddParam("@id", 100);
                    cmd.Parameters.AddParam("@displayNameId", 100);
        ...
        public static void GenerateActorAppearance()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Host={0}; Port={1}; Database={2}; Username={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    NpgsqlCommand cmd = new NpgsqlCommand(query, conn);
                    cmd.Parameters.AddParam("@id", 0);

                    for (int i = 0; i < NUMFIELDS; i++)
                        cmd.Parameters.AddParam("@v" + i, 100);
        ...
        public static void GenerateAchievementIds()
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(String.Format("Host={0}; Port={1}; Database={2}; Username={3}; Password={4}", ConfigConstants.DATABASE_HOST, ConfigConstants.DATABASE_PORT, ConfigConstants.DATABASE_NAME, ConfigConstants.DATABASE_USERNAME, ConfigConstants.DATABASE_PASSWORD)))
            {
                try
                {
                    ...
                    NpgsqlCommand cmd = new NpgsqlCommand(query, conn);
                    cmd.Parameters.AddParam("@id", 100);
                    cmd.Parameters.AddParam("@name", "Battle");
                    cmd.Parameters.AddParam("@otherId", 0);
                    cmd.Parameters.AddParam("@rewardPoints", 0);
                    ...
                    while ((line = file.ReadLine()) != null)
                    {
                        ...
                        Log.Debug("Wrote: {0} : {1} : {2} : {3}", id, name, otherId, points);
                        cmd.Parameters["@id"].Value = id;
            ...
            using (MemoryStream s = new MemoryStream(File.ReadAllBytes("D:\\luadec\\script\\staticactorr9w.luab")))
            {
                using (BinaryReader binReader = new BinaryReader(s))
                {
                    ...
                    using (StreamWriter w = File.AppendText("D:\\myfile.txt"))
                    {
                        ...
                        {
                            uint id = MeteorReborn.Common.Utils.SwapEndian(binReader.ReadUInt32()) | 0xA0F00000;
