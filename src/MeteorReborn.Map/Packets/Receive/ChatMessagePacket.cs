// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/ChatMessagePacket.cs (64 líneas).
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try{
                        ...
                        message = MeteorReborn.Common.Utils.ReadNullTermString(binReader, 0x200);
                    }
                    catch (Exception){
                        invalidPacket = true;
                    }
