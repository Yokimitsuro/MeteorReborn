// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/SupportDesk/GMSupportTicketPacket.cs (56 líneas).
...
namespace MeteorReborn.Map.Packets.Receive.SupportDesk
{
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try
                    {
                        ...
                        ticketTitle = MeteorReborn.Common.Utils.ReadNullTermString(binReader, 0x80);
                        ticketBody = MeteorReborn.Common.Utils.ReadNullTermString(binReader, 0x800);
                    }
                    catch (Exception){
                        invalidPacket = true;
                    }
