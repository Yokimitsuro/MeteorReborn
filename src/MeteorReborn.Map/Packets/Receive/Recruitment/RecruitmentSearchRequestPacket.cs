// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/Recruitment/RecruitmentSearchRequestPacket.cs (68 líneas).
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try{
                        ...
                        text = MeteorReborn.Common.Utils.ReadNullTermString(binReader);
                    }
                    catch (Exception){
                        invalidPacket = true;
                    }
