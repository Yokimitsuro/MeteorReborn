// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/Recruitment/StartRecruitingRequestPacket.cs (74 líneas).
...
namespace MeteorReborn.Map.Packets.Receive.Recruitment
{
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try{
                        ...
                        comment = MeteorReborn.Common.Utils.ReadNullTermString(binReader, 0x168);
                    }
                    catch (Exception){
                        invalidPacket = true;
                    }
