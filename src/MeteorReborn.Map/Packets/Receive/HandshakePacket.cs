// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/HandshakePacket.cs (52 líneas).
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try{
                        ...
                        actorID = UInt32.Parse(MeteorReborn.Common.Utils.ReadNullTermString(binReader, 10));
                    }
                    catch (Exception){
                        invalidPacket = true;
                    }
