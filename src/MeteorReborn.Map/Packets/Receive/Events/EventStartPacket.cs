// PM-COMPLETE → verbatim port de Map Server/Packets/Receive/Events/EventStartPacket.cs (89 líneas).
...
namespace MeteorReborn.Map.Packets.Receive.Events
{
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryReader binReader = new BinaryReader(mem))
                {
                    try{
                        ...
                        */
                        eventName = MeteorReborn.Common.Utils.ReadNullTermString(binReader);
