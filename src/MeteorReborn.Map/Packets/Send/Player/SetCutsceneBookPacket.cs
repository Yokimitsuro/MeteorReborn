// PM-COMPLETE → verbatim port de Map Server/Packets/Send/Player/SetCutsceneBookPacket.cs (116 líneas).
...
namespace MeteorReborn.Map.Packets.Send.Player
{
        ...
        public SubPacket BuildPacket(uint sourceActorId, string sNpcName, short sNpcActorIdOffset, byte sNpcSkin, byte sNpcPersonality)
        {
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryWriter binWriter = new BinaryWriter(mem))
                {
                    ...
                    MeteorReborn.Common.Utils.WriteNullTermString(binWriter, sNpcName);
                }
            }
