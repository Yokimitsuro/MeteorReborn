// PM-COMPLETE → verbatim port de Map Server/Packets/Send/Events/RunEventFunctionPacket.cs (61 líneas).
...
namespace MeteorReborn.Map.Packets.Send.Events
{
        ...
        public static SubPacket BuildPacket(uint triggerActorID, uint ownerActorID, string eventName, byte eventType, string functionName, List<LuaParam> luaParams)
        {
            ...
            using (MemoryStream mem = new MemoryStream(data))
            {
                using (BinaryWriter binWriter = new BinaryWriter(mem))
                {
                    ...
                    MeteorReborn.Common.Utils.WriteNullTermString(binWriter, eventName);
                    binWriter.Seek(0x29, SeekOrigin.Begin);                
                    MeteorReborn.Common.Utils.WriteNullTermString(binWriter, functionName);
