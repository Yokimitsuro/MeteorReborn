// PM-COMPLETE → verbatim port de Map Server/CommandProcessor.cs (148 líneas).
    ...
    class CommandProcessor
    {
        ...
        internal bool DoCommand(string input, Session session)
        {
            ...
            if (split[0].Equals("reloaditems"))
                {
                    Log.Information(String.Format("Got request to reload item gamedata"));
                    SendMessage(session, "Reloading Item Gamedata...");
                    gamedataItems.Clear();
                    gamedataItems = Database.GetItemGamedata();
                    Log.Information(String.Format("Loaded {0} items.", gamedataItems.Count));
                ...
                else if (split[0].Equals("property"))
                {
                    if (split.Length == 4)
                    {
                       // ChangeProperty(MeteorReborn.Common.Utils.MurmurHash2(split[1], 0), Convert.ToUInt32(split[2], 16), split[3]);
                    }
                    return true;
                }
                #endregion
