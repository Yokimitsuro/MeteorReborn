...
// PM-COMPLETE → verbatim port de Common Class Lib/SubPacket.cs (PM:22-239).
// LANG-ADAPT: namespace + NLog → System.Diagnostics.Debug (cambia transporte de log,
// comportamiento debug equivalente; PM-faithful porque PM solo usaba NLog para dump debug).
        ...
        public void DebugPrintSubPacket()
        {
            ...
            System.Diagnostics.Debug.WriteLine(
                string.Format("Size:0x{0:X} Opcode:0x{1:X}{2}{3}", header.subpacketSize, gameMessage.opcode,
                    Environment.NewLine,
                    Utils.ByteArrayToHex(GetHeaderBytes())));

            if (header.type == 0x03)
            {
                System.Diagnostics.Debug.WriteLine(Utils.ByteArrayToHex(GetGameMessageBytes(), SUBPACKET_SIZE));
                System.Diagnostics.Debug.WriteLine(Utils.ByteArrayToHex(data, SUBPACKET_SIZE + GAMEMESSAGE_SIZE));
            }
            else
                System.Diagnostics.Debug.WriteLine(Utils.ByteArrayToHex(data, SUBPACKET_SIZE));
#endif
        }
