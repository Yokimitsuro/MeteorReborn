...
// PM-COMPLETE → verbatim port de Common Class Lib/BasePacket.cs (PM:25-476).
// LANG-ADAPT: namespace + Ionic.Zlib → System.IO.Compression.ZLibStream (.NET 6+ tiene zlib
// nativo con header zlib, comportamiento idéntico al Ionic.Zlib.ZlibStream). NLog →
// System.Diagnostics.Debug.WriteLine (debug-only dump, equivalente).
...
namespace MeteorReborn.Common
{
    ...
    public class BasePacket
    {
        ...
        public static unsafe BasePacket CompressPacket(BasePacket uncompressedPacket)
        {
            using (var resultStream = new MemoryStream())
            {
                using (var zipStream = new ZLibStream(resultStream, CompressionMode.Compress, leaveOpen: true))
                {
                    zipStream.Write(uncompressedPacket.data, 0, uncompressedPacket.data.Length);
                }
    ...
    // PM-COMPLETE → LoggerExtensions class deliberadamente eliminada. PM la usaba para añadir
    // un método de extensión `ColorDebug` a NLog. MR no usa NLog (las llamadas a logger han
    // sido sustituidas por System.Diagnostics.Debug.WriteLine). LANG-ADAPT.
}
