// PM-COMPLETE → verbatim port de Common Class Lib/Vector3.cs (PM:22-180).
// LANG-ADAPT: namespace + nullable annotations en operators que usan `?.`. Sin cambios de
// comportamiento. NO se sustituye por `System.Numerics.Vector3` porque éste es struct y
// rompe los `lhs?.X == rhs?.X` checks + carece de GetAngle / NewHorizontalVector / IsWithin*.
