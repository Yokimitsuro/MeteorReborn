// PM-COMPLETE → verbatim port de Lobby Server/PacketProcessor.cs (PM:22-411).
// LANG-ADAPT: namespace + NLog `Program.Log` → Serilog `Log`.
// Dependencias futuras: `ClientConnection` (2.7) — fields `blowfish/currentUserId/currentSessionToken/
// newCharaCid/newCharaSlot/newCharaWorldId/newCharaName` + métodos `QueuePacket/GetAddress`.
...
using MeteorReborn.Lobby.Packets;
using MeteorReborn.Lobby.Packets.Receive;
using Serilog;
...
class PacketProcessor
{
        ...
        foreach (SubPacket subpacket in subPackets)
        {
            ...
            if (subpacket.header.type == 3)
            {
                switch (subpacket.gameMessage.opcode)
                {
                    ...
                    //Mod Retainers
                    default:
                        Log.Debug("Unknown command 0x{Opcode:X} received.", subpacket.gameMessage.opcode);
                        break;
                }
            }
        }
    ...
    private void ProcessStartSession(ClientConnection client, BasePacket packet)
    {
        ...
        Log.Information("SecCNum: 0x{ClientNumber:X}", securityHandshake.clientNumber);
    ...
    private void ProcessSessionAcknowledgement(ClientConnection client, SubPacket packet)
    {
        ...
        Log.Information("Got acknowledgment for secure session.");
        Log.Information("CLIENT VERSION: {ClientVersion}", clientVersion);
        ...
        if (userId == 0)
        {
            ...
            Log.Information("Invalid session, kicking...");
            return;
        }
    ...
    {
        Log.Information("{Who} => Get characters", client.currentUserId == 0 ? client.GetAddress() : "User " + client.currentUserId);
    ...
    private void ProcessSelectCharacter(ClientConnection client, SubPacket packet)
    {
        ...
        Log.Information("{Who} => Select character id {CharId}", client.currentUserId == 0 ? client.GetAddress() : "User " + client.currentUserId, selectCharRequest.characterId);
    ...
    private void ProcessModifyCharacter(ClientConnection client, SubPacket packet)
    {
        ...
        if (worldName == null)
        {
            ...
            Log.Information("User {UserId} => Error; invalid server id: \"{WorldId}\"", client.currentUserId, worldId);
            return;
        }
        ...
        switch (charaReq.command)
        {
            case 0x01://Reserve
                ...
                if (alreadyTaken)
                {
                    ...
                    Log.Information("User {UserId} => Error; name taken: \"{Name}\"", client.currentUserId, charaReq.characterName);
                    return;
                }
                else
                {
                ...
                Log.Information("User {UserId} => Character reserved \"{Name}\"", client.currentUserId, name);
                break;
            case 0x02://Make
                ...
                Log.Information("User {UserId} => Character Created \"{Name}\"", client.currentUserId, name);
                break;
            case 0x03://Rename
                ...
                Log.Information("User {UserId} => Character renamed \"{Name}\"", client.currentUserId, name);
                break;
            case 0x04://Delete
                Database.DeleteCharacter(charaReq.characterId, charaReq.characterName);

                Log.Information("User {UserId} => Character deleted \"{Name}\"", client.currentUserId, name);
                break;
            case 0x06://Rename Retainer

                Log.Information("User {UserId} => Retainer renamed \"{Name}\"", client.currentUserId, name);
                break;
        }
