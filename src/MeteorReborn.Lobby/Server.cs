// PM-COMPLETE → verbatim port de Lobby Server/Server.cs (PM:22-263).
...
class Server
{
    ...
    private PacketProcessor mProcessor = null!;
    ...
    {
        Log.Information("Cleanup thread started; it will run every {Seconds} seconds.", CLEANUP_THREAD_SLEEP_TIME);
        while (!killCleanupThread)
        {
            ...
            if (count != 0)
                Log.Information("{Count} connections were cleaned up.", count);
    ...
    public bool StartServer()
    {
        ...
        Log.Information("Lobby Server has started @ {Address}:{Port}", (mServerSocket.LocalEndPoint as IPEndPoint)!.Address, (mServerSocket.LocalEndPoint as IPEndPoint)!.Port);
    ...
    private void AcceptCallback(IAsyncResult result)
    {
        ...
        try
        {
            ...
            mServerSocket.BeginAccept(new AsyncCallback(AcceptCallback), mServerSocket);
            Log.Information("Connection {Address}:{Port} has connected.", (conn.socket.RemoteEndPoint as IPEndPoint)!.Address, (conn.socket.RemoteEndPoint as IPEndPoint)!.Port);
        }
    ...
    private void ReceiveCallback(IAsyncResult result)
    {
        ...
        try
        {
            ...
            {
                Log.Information("{Who} has disconnected.", conn.currentUserId == 0 ? conn.GetAddress() : "User " + conn.currentUserId);
